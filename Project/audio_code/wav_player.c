...
bool wavPlayer_fileSelect(const char* filePath)
{
  ...
  /* PCM + Stereo only */
  if(wavHeader.AudioFormat != 1 || wavHeader.NbrChannels != 2)
  {
    f_close(&wavFile);
    return false;
  }

  fileLength   = wavHeader.SubChunk2Size;
...
void wavPlayer_play(void)
{
  ...
  /* Skip WAV header */
  f_lseek(&wavFile, sizeof(WAV_HeaderTypeDef));

  f_read(&wavFile, audioBuffer, AUDIO_BUFFER_SIZE, &playerReadBytes);
  audioRemainSize = fileLength - playerReadBytes;

  audioI2S_play((uint16_t*)audioBuffer, AUDIO_BUFFER_SIZE);
}
